...
def all_tie(request, kid, reply_limit, time_limit):
    ...
    if request.method == 'GET':
        ...
        if kid == '0' and reply_limit == '0' and time_limit == '0':
            ...
            db = pymysql.connect(host='192.168.189.133', port=3306, user='root',
                                 password='wenka826*', db='forum_system')
            ...
            for topic in topics:
                ...
                count = 0
                print('query:',topic.id)
                cursor.callproc('query_r_count',[topic.id,count])
                cursor.execute("select @_query_r_count_1")  # 查询调用存储过程后返回的参数
                # db.commit()
